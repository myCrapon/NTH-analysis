...
% 平均管情况
Af = m*n*(GP^2-0.25*pi*d_cs^2)+4*n0*GP*(GP-d_cs)/2;     %总流通截面积(m^2)
tf_ = 0.5*(t_fin+t_fout);                               %冷却剂平均温度(℃)
rhof_ = refpropm('D','T',tf_+273.15,'P',Pres,'water');  %冷却剂平均密度(kg/m^3)
v_ = Wt*(1-Bf)/(Af*rhof_);                              %冷却剂平均流速(m/s)
Au = GP^2-0.25*pi*d_cs^2;                               %单元流通截面积(m^2)
Wu = Wt*(1-Bf)*Au/Af;                                   %单元截面流量(kg/s)
...
% 查找该压力温度下冷却剂的热物性
vis = refpropm('$','T',t_f2+273.15,'P',Pres,'water');   %运动粘度(m^2/s)
lamda = refpropm('L','T',t_f2+273.15,'P',Pres,'water'); %热导率(W/m·K)
Pr = refpropm('^','T',t_f2+273.15,'P',Pres,'water');    %普朗特数
t_s = refpropm('T','P',Pres,'Q',0,'water')-273.15;      %冷却剂饱和温度(℃)

Re = v_*De/(0.0001*vis);                                %计算该处的雷诺数
h = 0.023*(Re^0.8)*(Pr^0.4)*lamda/De;                   %该处的对流换热系数(W/m^2·K)
...
I_tu = UO2fit(t_u);                         %燃料芯块表面积分热导率(W/m)
